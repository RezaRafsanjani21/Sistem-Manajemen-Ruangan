...
@section('title', 'Buat Peminjaman - Sistem Manajemen Ruangan')
            ...
            <form method="POST" action="{{ route('peminjaman.store') }}" class="space-y-6">
                    ...
                    >
                        ...
                        @foreach($ruangan as $r)
                            <option value="{{ $r->ID_RUANGAN }}" {{ old('ID_RUANGAN') == $r->ID_RUANGAN ? 'selected' : '' }}>
                                {{ $r->NAMA_RUANGAN }} (Kapasitas: {{ $r->KAPASITAS }}) - {{ $r->LOKASI }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input 
                        ...
                        required 
                        maxlength="30"
                        value="{{ old('PENANGGUNG_JAWAB', auth()->user()->NAMA_USER) }}"
                        class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <div class="flex justify-end space-x-4">
                    <a href="{{ route('dashboard') }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition duration-200">
                        Batal
                    </a>
                    <button 
                        type="submit" 
